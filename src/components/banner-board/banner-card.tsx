...
export function BannerCard({
  ...
  const handleReload = () => {
    if (iframeRef.current && !isDataUrl) {
      ...
      setTimeout(() => {
        if(iframeRef.current) iframeRef.current.src = banner.url;
      }, 100);
    }
  };
    ...
    if (cardRef.current) {
      try {
        const canvas = await html2canvas(cardRef.current, {
          ...
          logging: false,
          onclone: (doc) => {
            // Attempt to make iframes visible in screenshot, might not work for cross-origin
            ...
            if (iframe) {
              try {
                ...
                if(iframeContent) {
                 ...
                 // cross-origin will throw error here
              }
            }
          }
        });
        ...
        toast({
          ...
          description: "Could not capture screenshot. The banner might be from an external domain.",
        });
      }
    }
  };
